...
def showMenu():
    ...
    label = Label(window, text="Extract info from PDF file(s) to json") # Instructions telling user to select a file or directory
    label.pack()

    button = Button(text = "Select Files", command = selectFiles) # Creats button for user to click after reading the instruction, when clicked selectDirOrFile will run
    button.pack()
    button.place(relx=0.35, rely=0.45, anchor=CENTER)

    button = Button(text = "Select Folder", command = selectDirectory) # Creats button for user to click after reading the instruction, when clicked selectDirOrFile will run
    button.pack()
    button.place(relx=0.66, rely=0.45, anchor=CENTER)

    buttonClose = Button(text = "Close", command = window.destroy) # Creats button for user to click after reading the instruction, when clicked selectDirOrFile will run
    buttonClose.pack()
